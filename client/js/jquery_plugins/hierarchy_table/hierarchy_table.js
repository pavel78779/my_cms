(function($){
    $.fn.hierarchyTable = function(params){
        var table = this.table(params),
            $table = this.children('table');

        //создаем ИЕРАРХИЮ
        $table.on('table_load', function(){
        ...
        //ИЗМЕНЕНИЕ ПОРЯДКА
        $table.off('.table_change_ordering').on('mousedown.hierarchy_table_change_ordering', 'div.change-ordering', function(e){
            e.preventDefault();
            ...
            var dragging = $('<table class="plugin-table dragging"></table>').css({
                width: $table.width(),
